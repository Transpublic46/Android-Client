...
import se.poochoo.cardsui.objects.AbstractCard;
import se.poochoo.cardsui.objects.Card;
import se.poochoo.cardsui.objects.CardStack;
...
public class CardUI extends FrameLayout {
    ...
    public void addCardAt(Card card, int index) {
        CardStack stack = new CardStack();
        stack.add(card);
        mStacks.add(index, stack);
    }

    public int addCard(Card card, boolean refresh) {
        CardStack stack = new CardStack();
        stack.add(card);
        mStacks.add(stack);
        if (refresh)
